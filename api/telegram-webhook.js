...
async function handleWebinarStart(msg, token, raw, res) {
  ...
  await sendTgMessage(
    msg.chat.id,
    `Готово, <b>${name}</b>! Вы записаны на вебинар <b>«Как расцвести после 40»</b> – 3 октября.

Ссылку на эфир пришлю сюда за день и за час до начала. Отвечать на это сообщение не нужно.`
  );
