...
export default async function handler(req, res) {
  ...
  if (!token) {
    await sendTgMessage(
      msg.chat.id,
      'Привет! Похоже, вы открыли бота напрямую. Чтобы пройти тест «Алхимия Женщины» — откройте <a href="https://alchemy-psi-five.vercel.app/test.html">эту страницу</a>, а после результата вернётесь сюда автоматически.'
    );
    return res.status(200).end();
  }
  ...
  if (!raw) {
    await sendTgMessage(
      msg.chat.id,
      'Похоже, ссылка устарела. <a href="https://alchemy-psi-five.vercel.app/test.html">Пройдите тест заново</a> — мы свяжемся в течение часа.'
    );
    return res.status(200).end();
  }
